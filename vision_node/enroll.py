    ...
    try:
        while True:
            ...
            if key == ord("s"):
                ...
                # Keep UI state in sync without doing expensive disk IO that can drop camera frames
                base_samples.extend(new_samples)
                if len(base_samples) > cfg.max_existing_crops:
                    base_samples = base_samples[-cfg.max_existing_crops:]
                new_samples.clear()
